<x-layouts.admin title="Brand Directory">
    ...
    <div class="space-y-4 mb-6">
        ...
        <div class="flex items-center justify-between gap-4">
            ...
            {{-- View toggle --}}
            <div class="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5" x-data="{ view: localStorage.getItem('brand-directory-view') || 'grid' }" x-init="$watch('view', v => localStorage.setItem('brand-directory-view', v))">
    ...
    @else
        {{-- Grid View --}}
        <div x-data="{ view: localStorage.getItem('brand-directory-view') || 'grid' }" x-init="$watch('view', v => localStorage.setItem('brand-directory-view', v))">
            ...
            <div x-show="view === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($brands as $brand)
                    <a href="{{ route('admin.brand-directory.edit', $brand) }}" class="group block">
                        <div class="relative bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                            {{-- Color accent strip --}}
                            <div class="h-1.5" style="background: {{ $brand->primary_color ?? '#D1D5DB' }}"></div>
                            ...
                            <div class="p-5">
                                ...
                                {{-- Info --}}
                                <h3 class="text-sm font-bold text-gray-900 group-hover:text-emerald-700 transition-colors">{{ $brand->name }}</h3>
                                @if ($brand->description)
                                    <p class="text-xs text-gray-400 mt-1 line-clamp-2 leading-relaxed">{{ $brand->description }}</p>
                                @endif
