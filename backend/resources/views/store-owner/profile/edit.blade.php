<x-layouts.store-owner title="Profile Settings">
    <x-slot:header>
        Settings
    </x-slot:header>
                        ...
                        <div class="lg:w-52 shrink-0"
                            x-data="{
                                ...
                                removeAvatar() {
                                    ...
                                    fetch('{{ route('store.profile.avatar.remove') }}', {
                                        method: 'DELETE',
                    ...
                    <div class="rounded-xl border border-gray-200 bg-white p-4">
                        ...
                        @unless ($user->phone_verified_at)
                            <div class="mt-3 pl-12 space-y-3" x-data="{ showOtp: {{ session('otp_code') ? 'true' : 'false' }} }">
                                <form method="POST" action="{{ route('store.profile.send-otp') }}" class="flex items-end gap-2">
                                    @csrf
                                    <div class="flex-1">
                                ...
                                <div x-show="showOtp" x-transition>
                                    <form method="POST" action="{{ route('store.profile.verify-otp') }}" class="flex items-end gap-2">
                                        @csrf
                                        <div class="flex-1">
