...
<head>
    ...
    <title>{{ $title ?? 'Agency Portal' }}</title>
    ...
    >
        <div class="flex items-center justify-between h-14 px-4 border-b border-gray-200/60 rounded-t-xl">
            <a href="{{ route('agency.dashboard') }}" class="flex items-center gap-2">
                ...
                <span class="font-semibold text-sm text-gray-900">Agency Portal</span>
            </a>
            <button
                type="button"
        ...
        @if ($agency)
            <div class="mx-3 mt-3 p-3 rounded-lg border border-gray-200/80 bg-gray-50/60">
                <div class="flex items-center gap-2.5">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-indigo-100">
                        <x-heroicon-s-truck class="w-4 h-4 text-indigo-600" />
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs font-semibold text-gray-900 truncate">{{ $agency->name }}</p>
                        <p class="text-[10px] text-gray-500 truncate">{{ $agency->collectors()->count() }} collectors</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto">
            @php
                $navItems = [
                    ['route' => 'agency.dashboard', 'label' => 'Dashboard', 'icon' => 'heroicon-o-home', 'exact' => true],
                    ['route' => 'agency.collectors.index', 'label' => 'Collectors', 'icon' => 'heroicon-o-users'],
                    ['route' => 'agency.routes.index', 'label' => 'Routes', 'icon' => 'heroicon-o-map'],
                    ['route' => 'agency.analytics', 'label' => 'Analytics', 'icon' => 'heroicon-o-chart-bar'],
                    ['route' => 'agency.profile.edit', 'label' => 'Settings', 'icon' => 'heroicon-o-cog-6-tooth'],
                ];
            @endphp
            ...
            @foreach ($navItems as $item)
                ...
                <a
                    ...
                    class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-100 {{ $isActive ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900' }}"
                >
        ...
        <header class="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-gray-200/60">
            <div class="flex items-center h-14 px-4 lg:px-6 gap-3">
                <div class="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <a href="{{ route('agency.dashboard') }}" class="shrink-0 flex items-center" title="Dashboard">
                        <img src="{{ asset('images/mobius-icon.png') }}" alt="Mobius" class="w-8 h-8 object-contain">
                    </a>
                    @if (isset($back))
                ...
                <div class="flex items-center gap-4 shrink-0">
                    ...
                    <div class="relative" data-topbar-dropdown>
                        <button type="button" data-topbar-trigger class="flex items-center justify-center h-10 rounded-full focus:outline-none cursor-pointer" title="User menu">
                            <span class="w-9 h-9 rounded-full bg-indigo-100 flex items-center justify-center text-xs font-semibold text-indigo-700 ring-1 ring-indigo-200/60 hover:ring-indigo-300 transition-all duration-150">
                        ...
                        >
                            <div class="flex items-center gap-3 px-4 py-1.5">
                                <span class="w-9 h-9 rounded-full bg-indigo-100 flex items-center justify-center text-sm font-semibold text-indigo-700 shrink-0">
                            ...
                            <a href="{{ route('agency.profile.edit') }}" class="topbar-dropdown-item">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var isPinned = localStorage.getItem('agency-sidebar-pinned') === '1';
    ...
    function updatePinUI() {
        if (isPinned) {
            ...
            if (pinBtn) { pinBtn.title = 'Unpin sidebar'; pinBtn.classList.add('text-indigo-600'); pinBtn.classList.remove('text-gray-400'); }
        } else {
            document.body.classList.remove('sidebar-pinned');
            if (pinLabel) pinLabel.textContent = 'Pin sidebar';
            if (pinBtn) { pinBtn.title = 'Pin sidebar'; pinBtn.classList.remove('text-indigo-600'); pinBtn.classList.add('text-gray-400'); }
        }
    }
    ...
    if (pinBtn) {
        pinBtn.addEventListener('click', function (e) {
            e.preventDefault();
            isPinned = !isPinned;
            localStorage.setItem('agency-sidebar-pinned', isPinned ? '1' : '0');
            updatePinUI();
