<?php

namespace App\Services;

use Illuminate\Http\UploadedFile;
use Illuminate\Support\Facades\Storage;

class AvatarService
{
    private const SIZE = 512;

    private const QUALITY = 85;

    /**
     * Process an uploaded avatar: center-crop to square, resize to 512x512, compress as JPEG.
     * Returns the stored path relative to the public disk.
     */
    public function process(UploadedFile $file): string
    {
        $source = $this->createImageFromFile($file);
        $width = imagesx($source);
        $height = imagesy($source);

        // Center-crop to square
        $side = min($width, $height);
        $x = (int) (($width - $side) / 2);
        $y = (int) (($height - $side) / 2);

        $cropped = imagecreatetruecolor($side, $side);
        imagecopyresampled($cropped, $source, 0, 0, $x, $y, $side, $side, $side, $side);
        imagedestroy($source);

        // Resize to standard dimensions
        $final = imagecreatetruecolor(self::SIZE, self::SIZE);
        imagecopyresampled($final, $cropped, 0, 0, 0, 0, self::SIZE, self::SIZE, $side, $side);
        imagedestroy($cropped);

        // Save as JPEG to a temp file, then store
        $tempPath = tempnam(sys_get_temp_dir(), 'avatar_');
        imagejpeg($final, $tempPath, self::QUALITY);
        imagedestroy($final);

        $filename = 'avatars/'.uniqid('av_').'.jpg';
        Storage::disk('public')->put($filename, file_get_contents($tempPath));
        unlink($tempPath);

        return $filename;
    }

    /**
     * Delete an existing avatar from storage.
     */
    public function delete(?string $path): void
    {
        if ($path) {
            Storage::disk('public')->delete($path);
        }
    }

    private function createImageFromFile(UploadedFile $file): \GdImage
    {
        $mime = $file->getMimeType();
        $path = $file->getPathname();

        $image = match ($mime) {
            'image/jpeg' => imagecreatefromjpeg($path),
            'image/png' => imagecreatefrompng($path),
            'image/gif' => imagecreatefromgif($path),
            'image/webp' => imagecreatefromwebp($path),
            default => imagecreatefromjpeg($path),
        };

        if ($image === false) {
            throw new \RuntimeException('Failed to read uploaded image.');
        }

        return $image;
    }
}
