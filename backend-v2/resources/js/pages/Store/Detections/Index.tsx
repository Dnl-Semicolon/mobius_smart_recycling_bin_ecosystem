...
export default function StoreDetectionsIndex({
    events,
    filters,
}: {
    events: Paginated;
    ...
    function filter(value: string) {
        router.get('/store/detections', { waste_type: value || undefined }, { preserveState: true });
    }
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">{events.total} detections across your outlets</p>
                    </div>
                    <select
                            ...
                            {events.data.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={7} className="py-8 text-center text-muted-foreground">
                                        No detection events for your outlets yet.
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
...
StoreDetectionsIndex.layout = {
    breadcrumbs: [
        { title: 'Store Dashboard', href: '/store' },
        { title: 'Detection Events', href: '/store/detections' },
    ],
};
