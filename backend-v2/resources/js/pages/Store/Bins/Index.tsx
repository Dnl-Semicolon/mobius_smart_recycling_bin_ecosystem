...
export default function StoreBinsIndex({ bins }: { bins: Paginated }) {
    function requestPickup(binId: number) {
        router.post(`/store/bins/${binId}/request-pickup`, {}, { preserveScroll: true });
    }
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">My Bins</h1>
                        <p className="text-sm text-muted-foreground">Bins across your outlets</p>
                    </div>
                </div>
                                ...
                                <TableRow key={bin.id}>
                                    ...
                                    <TableCell>
                                        <Button
                                            size="sm"
                                            variant="outline"
                                            disabled={bin.has_pending_pickup || bin.status !== 'active'}
                            ...
                            {bins.data.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={6} className="py-8 text-center text-muted-foreground">
                                        No bins assigned to your outlets.
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
...
StoreBinsIndex.layout = {
    breadcrumbs: [
        { title: 'Store Dashboard', href: '/store' },
        { title: 'Bins', href: '/store/bins' },
    ],
};
